...
import { useMySQLBooks } from '../context/MySQLBooksContext';
import { useMySQLVideos } from '../context/MySQLVideosContext';
...
const Home = () => {
  ...
  const { t } = useLanguage();
  const { books, loading: booksLoading } = useMySQLBooks();
  const { videos, loading: videosLoading } = useMySQLVideos();
  ...
  useEffect(() => {
    const calculateCategories = () => {
      ...
      const categoriesWithCounts = Array.from(allCategories).map((categoryName, index) => {
        const videoCount = publicVideos.filter(video => video.category === categoryName).length;
        const bookCount = publicBooks.filter(book => book.category === categoryName).length;
  ...
  const displayVideos = (() => {
    // If there are filters applied, show filtered results from all public videos
    if (searchQuery || selectedCategory) {
      ...
      // If no filters, show public videos
      return publicVideos;
    }
  })().sort((a, b) => {
    switch (sortBy) {
  ...
  if (booksLoading || videosLoading) {
    return (
      <div className="home">
        <div className="container">
          <div className="loading-state">
            <h2>Loading content...</h2>
            <p>Please wait while we fetch your books and videos.</p>
          </div>
        </div>
      </div>
    );
